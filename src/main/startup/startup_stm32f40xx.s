/**
  ...
  *            3.设置中断向量表的对应中断地址;
  *            4.设置程序入口函数为main.
  *            在进入main函数前所进行的操作，主要是初始化SP与PC指针，以及中断向量表.
  ******************************************************************************
  */
...
/* global使得g_pfnVectors可以被其他目标文件使用 */
.global  g_pfnVectors
.global  Default_Handler

/* 数据段初始化值的起始地址，在链接器脚本ld文件中定义 */
.word  _sidata
/* 数据段的起始地址，在链接器脚本ld文件中定义 */
.word  _sdata
/* 数据段的结束地址，在链接器脚本ld文件中定义 */
.word  _edata
/* bss段的起始地址，在链接器脚本ld文件中定义 */
.word  _sbss
/* bss段的结束地址，在链接器脚本ld文件中定义 */
.word  _ebss
/* fastram_bss段的起始地址，在链接器脚本ld文件中定义 */
.word  __fastram_bss_start__
/* fastram_bss段的结束地址，在链接器脚本ld文件中定义 */
.word  __fastram_bss_end__

/*********************************************************
** 这是处理器第一次调用的代码,在重置事件后开始执行
** 作用：
...
/******************************************************************************
** Cortex M4的最小向量表，注意正确的结构
** 必须放在这个上，以确保它结束在物理地址0x0000.0000.
*******************************************************************************/
...
** 在有中断被触发后，如果没有重写对应的弱别名
** 那么程序就默认执行默认的中断处理函数（Default_Handler）
** 反之则执行重写了的中断处理函数
*******************************************************************************/
   .weak      NMI_Handler
